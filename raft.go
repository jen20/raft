	...
	for {
		...
		case <-heartbeatTimer:
			...
			if r.configurations.latestIndex == 0 {
				...
				r.logger.Warn(`Heartbeat timeout from reached, starting election`, "leader", lastLeader)
...
func (r *Raft) runCandidate() {
	r.logger.Info("entering Candidate state", "peer", r, "term", r.getCurrentTerm()+1)
	...
	for r.getState() == Candidate {
		...
		case vote := <-voteCh:
			...
			if vote.Granted {
				grantedVotes++
				r.logger.Debug("Vote granted from peer", "peer", vote.voterID,
...
func (r *Raft) runLeader() {
	r.logger.Info("Entering Leader state", "peer", r)
...
func (r *Raft) startStopReplication() {
	...
	for serverID, repl := range r.leaderState.replState {
		...
		r.logger.Info("Removed peer, stopping replication", "peer", serverID, "index", lastIdx)
		repl.stopCh <- lastIdx
		close(repl.stopCh)
	...
	for peer, f := range r.leaderState.replState {
		...
		if diff <= r.conf.LeaderLeaseTimeout {
			...
			if diff <= 3*r.conf.LeaderLeaseTimeout {
				r.logger.Warn("Failed to contact peer within timeout", "peer", peer, "timeout", diff)
			} else {
				r.logger.Debug("Failed to contact peer within timeout", "peer", peer, "timeout", diff)
			}
		}
...
func (r *Raft) restoreUserSnapshot(meta *SnapshotMeta, reader io.Reader) error {
	...
	r.logger.Info("Copied data to local snapshot", "bytes", n)
...
func (r *Raft) appendConfigurationEntry(future *configurationChangeFuture) {
	...
	r.logger.Info("Updating configuration", "command", future.req.command,
		"peer", future.req.serverID, "peer_address", future.req.serverAddress, "new_peers", configuration.Servers)
	...
	if a.PrevLogEntry > 0 {
		...
		if a.PrevLogEntry == lastIdx {
			...
			if err := r.logs.GetLog(a.PrevLogEntry, &prevLog); err != nil {
				r.logger.Warn("Failed to get previous log", "previous_log_entry", a.PrevLogEntry,
					"last_index", lastIdx, "error", err)
		...
		if a.PrevLogTerm != prevLogTerm {
			r.logger.Warn("Previous log term mis-match", "local_term", prevLogTerm,
				"remote_term", a.PrevLogTerm)
			resp.NoRetryBackoff = true
			return
		}
	}
	...
	if len(a.Entries) > 0 {
		...
		for i, entry := range a.Entries {
			...
			if entry.Term != storeEntry.Term {
				r.logger.Warn("Clearing log suffix", "index", entry.Index, "last_index", lastLogIdx)
...
func (r *Raft) requestVote(rpc RPC, req *RequestVoteRequest) {
	...
	if lastVoteTerm == req.Term && lastVoteCandBytes != nil {
		...
		if bytes.Compare(lastVoteCandBytes, req.Candidate) == 0 {
			r.logger.Warn("Duplicate RequestVote from candidate", "candidate", req.Candidate)
			resp.Granted = true
		}
		return
	}
	...
	if lastTerm > req.LastLogTerm {
		r.logger.Warn("Rejecting vote request since our last term is greater",
			"candidate", candidate, "local_term", lastTerm, "remote_term", req.LastLogTerm)
		return
	}

	if lastTerm == req.LastLogTerm && lastIdx > req.LastLogIndex {
		r.logger.Warn("Rejecting vote request from %v since our last index is greater",
			"candidate", candidate, "local_last_index", lastIdx, "remote_last_index", req.LastLogIndex)
		return
	}
...
func (r *Raft) installSnapshot(rpc RPC, req *InstallSnapshotRequest) {
	...
	// Ignore an older term
	if req.Term < r.getCurrentTerm() {
		r.logger.Info("Ignoring installSnapshot request with older term vs current term",
			"local_term", req.Term, "remote_term", r.getCurrentTerm())
		return
	}
	...
	if n != req.Size {
		sink.Cancel()
		r.logger.Error("Failed to receive whole snapshot", "bytes_received", n, "total_bytes", req.Size)
...
func (r *Raft) electSelf() <-chan *voteResult {
	...
	askPeer := func(peer Server) {
		r.goFunc(func() {
			...
			if err != nil {
				r.logger.Error("Failed to make RequestVote RPC", "peer", peer, "error", err)
				resp.Term = req.Term
				resp.Granted = false
			}
			respCh <- resp
