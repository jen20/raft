...
func (r *Raft) takeSnapshot() (string, error) {
	...
	r.logger.Info("Starting snapshot up to index", "index", snapReq.index)
	start := time.Now()
	...
	r.logger.Info("Snapshot to index complete", "index", snapReq.index)
	return sink.ID(), nil
}
...
func (r *Raft) compactLogs(snapIdx uint64) error {
	...
	// Log this
	r.logger.Info("Compacting logs", "min_index", minLog, "max_index", maxLog)
